...
def extract_adjectives(lst):
    ...
    for ind, word in enumerate(lst):  
        if word['POS'] == 'NOUN':
            noun_indexes.append(ind)  # записывает индексы существительных
    dct = {}
    for ind, word in enumerate(lst):
        if word['POS'] == 'ADJF':  # находит прилагательные
            noun = False
            for noun_ind in noun_indexes:  # сверяет индексы прилагательных с индексами существительных
                if noun_ind - ind == 1:
                    noun = lst[noun_ind]
                elif noun_ind - ind == 2:
                    noun = lst[noun_ind]
            if noun \
                    ...
                    and word['gender'] == noun['gender']:  # проверяет согласование существительного и прилагательного
                if word['year'] in dct:
...
def sort_adjectives(dct):
    ...
    for year, inner_dict in dct.items():
        for noun, inner_list in inner_dict.items():
            ...
            for adj in adj_dict:
                adj_dict[adj] = inner_list.count(adj)  # считает количество одинаковых прилагательных для каждого существительного
            adj_dict = sorted(adj_dict, key=adj_dict.get, reverse=True)  # сортирует по количеству повторений
